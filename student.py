...
class StudentPlayer(Player):
    ...
    def bet(self, dealer, players):
        ...
        if self.win_streak == 1:
            self.bet_value = self.rules.min_bet + 1
        ...
        if self.win_streak == 2:
            self.bet_value = self.rules.min_bet + 2
            if self.bet_value > self.rules.max_bet:
                self.bet_value = self.rules.max_bet
                if self.lose_streak > 3:
                    self.bet_value = self.rules.min_bet
        ...
        if self.win_streak == 3:
            self.bet_value = self.rules.min_bet + 3
            if self.bet_value > self.rules.max_bet:
                self.bet_value = self.rules.max_bet
                if self.lose_streak > 3:
                    self.bet_value = self.rules.min_bet
        ...
        if self.win_streak == 4:
            self.bet_value = self.rules.min_bet + 4
            if self.bet_value > self.rules.max_bet:
                self.bet_value = self.rules.max_bet
                if self.lose_streak > 3:
                    self.bet_value = self.rules.min_bet
        ...
        '''self.bet_value = self.fibonacci[self.win_streak]
        if self.bet_value < self.rules.min_bet:
            self.bet_value = self.rules.min_bet
        elif self.bet_value > self.rules.max_bet:
            self.bet_value = self.rules.max_bet
        return self.bet_value'''
        ...
        '''self.bet_value = self.parlay[self.win_streak]
        if self.bet_value < self.rules.min_bet:
            self.bet_value = self.rules.min_bet
        elif self.bet_value > self.rules.max_bet:
            self.bet_value = self.rules.max_bet
        ...
        if self.pocket > self.initmoney * 2:             #200
            self.bet_value = self.rules.max_bet          #5
            if self.lose_streak > 2:
                self.bet_value = self.rules.min_bet+2    #3
                return self.bet_value
            if self.lose_streak > 3:
                self.bet_value = self.rules.min_bet+1    #2
                return self.bet_value
        elif self.pocket > self.initmoney * 1.7:         #150
            self.bet_value = self.rules.max_bet -1       #4
            if self.lose_streak > 4:
                self.bet_value = self.rules.min_bet+2    #3
                return self.bet_value
            if self.lose_streak > 5:
                self.bet_value = self.rules.max_bet    #5 negative progress
                return self.bet_value
        elif self.pocket > self.initmoney * 1.2:        #120
            self.bet_value = self.rules.max_bet -2       #3
        elif self.pocket > self.initmoney * 1.1:        #111
            self.bet_value = self.rules.max_bet * 0.3    #2
    ...
    def payback(self, prize):
        ...
        win = False
        if prize >= self.bet_value:  # win
            self.wins += 1
            win = True
            self.win_streak += 1
            self.lose_streak = 0
            self.win_streak_number += 1
            self.win_profit += prize
        elif prize == 0:            # tie
            self.ties += 1
            self.win_streak += 0
        else:                       # defeat
